...
const RELEASE_BASE =
  "https://github.com/Complexity-ML/labo-ai/releases/download/v0.1.0";
        ...
        <div className="site-shell relative">
          <div className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
              The PyTorch runtime stays on your machine. Add your own OpenAI API key only when you want to use Ask LABO.
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2">
            <article className="rounded-2xl border border-white/10 bg-card/80 p-7">
              ...
              <Button className="mt-7 w-full bg-white text-black hover:bg-white/85" asChild>
                <a href={`${RELEASE_BASE}/LABO-AI-0.1.0-arm64.dmg`}>
                  <Download className="size-4" /> Download DMG
                </a>
              </Button>
              <Button variant="ghost" className="mt-2 w-full text-white/55" asChild>
                <a href={`${RELEASE_BASE}/LABO-AI-0.1.0-arm64-mac.zip`}>Portable ZIP</a>
              </Button>
            </article>
            ...
            <article className="rounded-2xl border border-white/10 bg-card/80 p-7">
              ...
              <Button className="mt-7 w-full bg-sky-500 text-white hover:bg-sky-400" asChild>
                <a href={`${RELEASE_BASE}/LABO-AI-0.1.0-x64.exe`}>
                  <Download className="size-4" /> Download installer
                </a>
              </Button>
              <Button variant="ghost" className="mt-2 w-full text-white/55" asChild>
                <a href={`${RELEASE_BASE}/LABO-AI-0.1.0-x64-win.zip`}>Portable ZIP</a>
              </Button>
            </article>
          </div>

          <div className="mt-5 flex flex-col items-center justify-between gap-4 rounded-2xl border border-white/10 bg-black/20 px-6 py-5 text-sm sm:flex-row">
            <div className="flex items-center gap-3 text-white/65">
              <ShieldCheck className="size-5 text-emerald-300" />
              Open source. API keys are encrypted with Electron safeStorage.
            </div>
            <Button variant="ghost" size="sm" asChild>
              <a href="https://github.com/Complexity-ML/labo-ai/releases/tag/v0.1.0" target="_blank" rel="noopener noreferrer">
                All downloads <ExternalLink className="size-3.5" />
              </a>
            </Button>
          </div>

          <div className="mt-10 grid gap-3 text-sm text-white/50 sm:grid-cols-3">
            {["Typed graph validation", "Encrypted local credentials", "Inspectable generated code"].map((item) => (
              <div key={item} className="flex items-center justify-center gap-2">
                <Check className="size-3.5 text-emerald-300" /> {item}
              </div>
            ))}
          </div>
        </div>
